...
TEST(PCM, ConsistencyCheck)
{
  ...
  // Check everything is normal here 
  EXPECT(gtsam::assert_equal(size_t(7), nfg.size()));
  EXPECT(gtsam::assert_equal(size_t(7), est.size()));
  ...
  EXPECT(gtsam::assert_equal(size_t(9), nfg.size()));
  EXPECT(gtsam::assert_equal(size_t(7), est.size()));
  EXPECT(do_optimize == true); 
  ...
  EXPECT(gtsam::assert_equal(size_t(10), nfg.size()));
  EXPECT(gtsam::assert_equal(size_t(7), est.size()));
  EXPECT(do_optimize == true);
