...
TEST(RobustSolver, multiRobotPcm)
{
  ...
  init_factors_b.add(gtsam::BetweenFactor<gtsam::Pose3>(init_key_a, init_key_b, tf_ab, noise));
  ...
  for (size_t i = 0; i < 3; i++) {
    ...
    odom_factor_a.add(gtsam::BetweenFactor<gtsam::Pose3>(key_a_prev, key_a_new, odom, noiseOdom));
    odom_factor_b.add(gtsam::BetweenFactor<gtsam::Pose3>(key_b_prev, key_b_new, odom, noiseOdom));
  ...
  R.row(1) << 0, -1, 0;
  R.row(2) << 0, 0, 1;
  lc_factors.add(gtsam::BetweenFactor<gtsam::Pose3>(b4, a4,
      gtsam::Pose3(gtsam::Rot3(R), gtsam::Point3(-1,0,0)), noise));
  ...
  lc_factors.add(gtsam::BetweenFactor<gtsam::Pose3>(a2, b2,
      gtsam::Pose3(gtsam::Rot3(), gtsam::Point3(0,-1,0)), noise));

  //bad lc
  gtsam::Key a5 = gtsam::Symbol('a',5);
  gtsam::Key b5 = gtsam::Symbol('b',5);
  lc_factors.add(gtsam::BetweenFactor<gtsam::Pose3>(b5, a5,
      gtsam::Pose3(gtsam::Rot3(), gtsam::Point3(0,-3.3,0)), noise));
  ...
  R.row(1) << -1, 0, 0;
  R.row(2) << 0, 0, 1;
  lc_factors.add(gtsam::BetweenFactor<gtsam::Pose3>(a4, a1,
      gtsam::Pose3(gtsam::Rot3(R), gtsam::Point3(0,3,0)), noise));
  // bad lc
  lc_factors.add(gtsam::BetweenFactor<gtsam::Pose3>(a4, a2,
      gtsam::Pose3(gtsam::Rot3::Rz(-1.54), gtsam::Point3(0,2,0)), noise));
