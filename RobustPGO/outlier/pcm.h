  ...
  Pcm(double threshold1, double threshold2,
      ...
      : OutlierRemoval(),
        threshold1_(threshold1), // TODO(Luca): threshold1 and 2 seem too
                                 // generic as names
        threshold2_(threshold2), special_symbols_(special_symbols) {
  ...
  virtual bool removeOutliers(const gtsam::NonlinearFactorGraph &new_factors,
    ...
    if (trajectory_odom_.poses.size() == 0) {
      if (new_values.size() == 1 &&
        ...
        if (debug_)
          log<INFO>("Initializing without prior");
        ...
        if (debug_)
          log<INFO>("Initializing with prior");
        ...
        log<WARNING>("Unhandled initialization: first time PCM is called, it "
                     "needs a particular input");
        return false;
      }
      if (debug_)
        log<INFO>("Initialized trajectory");
    }
    ...
    case FactorType::LOOP_CLOSURES: {
      parseAndIncrementAdjMatrix(
          new_factors,
          output_values); // output values is just used to sanity check the keys
      findInliers(); // update inliers, // Find inliers with Pairwise consistent
                     // measurement set maximization
      doOptimize = true;
    } break;
...
protected:
   ...
   */
  void
  parseAndIncrementAdjMatrix(const gtsam::NonlinearFactorGraph &new_factors,
  ...
  void updateOdom(const gtsam::NonlinearFactorGraph &new_factors) {
    if (new_factors.size() != 1) {
      log<WARNING>("Factors passed to updateOdom should be of size one (the "
                   "odom factor)");
    }
    ...
    try {
      ...
      // TODO: this in the future can be where we initialize key0! for future
      // releases
    }
  ...
  bool checkOdomConsistent(const PoseWithCovariance<poseT> &result,
    ...
    if (debug_)
      log<INFO>("odometry consistency distance: %1%") % dist;
    if (dist < threshold1_) {
      return true;
    }
    return false;
  ...
  void incrementAdjMatrix() {
    ...
    size_t num_lc = nfg_lc_.size(); // number of loop closures so far, including
    ...
    if (num_lc > 1) {
      ...
      for (size_t i = 0; i < num_lc - 1; i++) {
        ...
        if (keyi == ldmk_key || keyj == ldmk_key) {
          log<WARNING>("Landmark observations should be connected pose -> "
                       "landmark, discarding");
          return;
        }
