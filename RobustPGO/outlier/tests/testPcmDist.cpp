...
/* ************************************************************************* */
TEST(PcmDistance, OdometryCheck)
{
  // Here want to test carefully pcm
  // first test odometry check so set pcm thres high
  OutlierRemoval *pcm = new PcmDistance3D(0.053, 100.0);
  pcm->setQuiet();
  ...
  for (size_t i = 0; i < 3; i++) {
    ...
    odom_factor.add(gtsam::BetweenFactor<gtsam::Pose3>(i, i+1, odom, noiseOdom));
    pcm->process(odom_factor, odom_val, nfg, est);
  }
  ...
  // Then add loop closure 1 with percentage error of around 0.0526
  ...
  bool do_optimize = pcm->process(lc_factor1, gtsam::Values(), nfg, est);
  EXPECT(size_t(5)==nfg.size());
  EXPECT(size_t(4)==est.size());
  EXPECT(do_optimize == true);

  // Then add loop closure 2 with percentage dist of around 0.054
...
/* ************************************************************************* */
TEST(PcmDistance, ConsistencyCheck)
{
  // Here want to test carefully pcm
  // test pcm check so set odom thres high
  OutlierRemoval *pcm = new PcmDistance3D(100.0, 0.05);
  // pcm->setQuiet();
  ...
  // pcm->process(lc_factor1, gtsam::Values(), nfg, est);
  ...
  // bool do_optimize = pcm->process(lc_factor2, gtsam::Values(), nfg, est);
  ...
  // do_optimize = pcm->process(lc_factor3, gtsam::Values(), nfg, est);
  ...
  // do_optimize = pcm->process(lc_factor4, gtsam::Values(), nfg, est);
...
/* ************************************************************************* */
TEST(PcmDistance, OdometryCheck2D)
{
  // TODO
}

/* ************************************************************************* */
TEST(PcmDistance, ConsistencyCheck2D)
{
  // TODO
}
