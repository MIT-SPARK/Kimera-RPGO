...
TEST(RobustSolver, LandmarkPcm)
{
  ...
  for (size_t i = 0; i < 2; i++) {
    ...
    odom_factor.add(gtsam::BetweenFactor<gtsam::Pose3>(key_prev, key_new, odom, noiseOdom));
  ...
  lc_factors.add(gtsam::BetweenFactor<gtsam::Pose3>(a3, a2,
      gtsam::Pose3(gtsam::Rot3::Rz(-1.57), gtsam::Point3(0,0.9,0)), noise));

  lc_factors.add(gtsam::BetweenFactor<gtsam::Pose3>(a4, a1,
      gtsam::Pose3(gtsam::Rot3::Rz(3.14), gtsam::Point3(2.1,1.1,2.5)), noise));
  ...
  landmark_values.insert(l0, gtsam::Pose3(gtsam::Rot3(), gtsam::Point3(1, 1, 0)));
