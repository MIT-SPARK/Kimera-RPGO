...
int main(int argc, char *argv[]) {
  ...
  // const int& d = const_cast<int&>(graph_utils::getDim<gtsam::Pose3>());
  const int d = 6; 
  gtsam::Matrix covar = Eigen::MatrixXd::Zero(10,10);
  covar.block<d,d>(0,0) = Eigen::MatrixXd::Identity(d,d);

  OutlierRemoval *pcm = new PCM<gtsam::Pose3>(100.0, 0.5); // with debug set to true
